                    ...
                    <div class="row">
                        <div class="col-12 col-md-12">
                            <div class="card redial-border-light redial-shadow mb-4">
                                <div class="card-body">
                                    <h6 class="header-title pr-3 redial-relative">تعديل المقال</h6>
                                    <form method="post">
                                        <div class="form-group" style="display: none;">
                                            <input type="number" required name="article_id"
                                                value="<?= $data['current_article']->id ?>" />
                                        </div>
                                        <div class="form-group">
                                            <label class="redial-font-weight-600">عنوان</label>
                                            <input type="text" required class="form-control" name="title"
                                                placeholder="عنوان" value="<?= $data['current_article']->title ?>" />
                                        </div>
                                        <div class="form-group">
                                            <label class="redial-font-weight-600">محتوى المقال</label>
                                            <textarea class="form-control" placeholder="محتوى المقال"
                                                name="content"><?= $data['current_article']->content ?></textarea>
                                        </div>
                                        <div class="form-group">
                                            <label class="redial-font-weight-600">اختر قسم المقال</label>
                                            <select class="form-control" name="type">
                                                <option value=<?= JOMOAA ?>
                                                    <?= ($data['current_article']->type == JOMOAA) ? 'selected' : '' ?>>
                                                    خطب الجمعة</option>
                                                <option value=<?= TAFSSIR ?>
                                                    <?= ($data['current_article']->type == TAFSSIR) ? 'selected' : '' ?>>
                                                    تفسير الاحلام</option>
                                                <option value=<?= ROQIA ?>
                                                    <?= ($data['current_article']->type == ROQIA) ? 'selected' : '' ?>>
                                                    الرقية الشرعية</option>
                                            </select>
                                        </div>

                                        <div class="redial-divider my-4"></div>
                                        <button type="submit" class="btn btn-primary btn-xs"
                                            name="edit_article">حفظ</button>
                                    </form>
                                </div>
                            </div>
                        </div>
