                                ...
                                <div class="card-body">
                                    ...
                                    <form method="post" enctype="multipart/form-data">
                                        <div class="form-group" style="display: none;">
                                            <input type="number" required name="asset_id"
                                                value="<?= $data['current_asset']->id ?>" />
                                        </div>
                                        <div class="form-group">
                                            <label class="redial-font-weight-600">عنوان</label>
                                            <input type="text" required class="form-control" name="title"
                                                placeholder="عنوان" value="<?= $data['current_asset']->title ?>" />
                                        </div>
                                        <div class="form-group">
                                            <label class="redial-font-weight-600">الوصف</label>
                                            <textarea class="form-control" placeholder="الوصف"
                                                name="description"><?= $data['current_asset']->description ?></textarea>
                                        </div>
                                        <div class="form-group">
                                            <label class="redial-font-weight-600">اختر نوع المحتوى</label>
                                            <select class="form-control" name="type">
                                                <option value=<?= VIDEOS ?>
                                                    <?= ($data['current_asset']->type == VIDEOS) ? 'selected' : '' ?>>
                                                    فيديوهات</option>
                                                <option value=<?= IMAGES ?>
                                                    <?= ($data['current_asset']->type == IMAGES) ? 'selected' : '' ?>>
                                                    صور</option>
                                                <option value=<?= SOUNDS ?>
                                                    <?= ($data['current_asset']->type == SOUNDS) ? 'selected' : '' ?>>
                                                    اصوات</option>
                                            </select>
                                        </div>
                                        <div class="form-group">
