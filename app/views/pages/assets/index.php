                            ...
                            <div class="card redial-border-light redial-shadow">
                                <div class="card-body">
                                    <h6 class="header-title pr-3 redial-relative">
                                        <?= (@$_GET['type'] == VIDEOS ? 'فيديوهات' : (@$_GET['type'] == IMAGES ? 'صور' : 'اصوات')) ?>
                                    </h6>
                                    <table class="table table-striped table-hover mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><?= $index ?></td>
                                                <td><?= $asset->title ?></td>
                                                <td><?= substr($asset->description, 0, 80) ?>..</td>
                                                <td><a href="<?= URLROOT . $asset->file_content ?>" type="button"
                                                        target="_blank" class="btn btn-info">معاينة</a></td>
                                                <td>
                                                    <a href="<?= URLROOT . 'pages/assets/edit&asset_id=' . $asset->id ?>"
                                                        type="button" class="btn btn-primary">تعديل</a>
                                                    <button type="button" class="btn btn-danger" data-toggle="modal"
                                                        data-target="#model<?= $asset->id ?>">حذف</button>
                                                </td>

                                            </tr>
                                            <div class="modal fade" id="model<?= $asset->id ?>" tabindex="-1"
                                                ...
                                                <div class="modal-dialog  modal-md" role="document">
                                                    <div class="modal-content redial-border-light">
                                                        ...
                                                        <div class="modal-body">
                                                            <p>هل تريد حذف: <?= '<b>' . $asset->title . '</b>' ?>
                                                            </p>
                                                        </div>
                                                        <div class="modal-footer redial-border-light">
                                                            <button type="button" class="btn btn-secondary"
                                                                data-dismiss="modal">الغاء</button>
                                                            <a href="<?= URLROOT . 'pages/assets/del&asset_id=' . $asset->id ?>"
